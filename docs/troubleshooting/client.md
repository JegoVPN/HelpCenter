...
先在[客户端教程](/tool/)中找到你正在使用的工具，确认安装、导入订阅、选择节点和打开连接的步骤都做过。然后按下面最接近的现象排查。

## 1. 订阅无法导入

确认这个客户端适合你的[设备](/devices/)，再从无忧行控制面板重新复制一次订阅。按工具教程选择正确的订阅类型，并直接使用控制面板生成的原始链接。提示格式错误时，记下错误文字。
