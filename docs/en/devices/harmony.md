...
On older HarmonyOS releases, some users ran Android clients through a compatibility layer. Prefer Huawei AppGallery or the project's official source and keep system protection enabled.

### Historical interface reference: Pure Mode and Enhanced Protection

The screenshots below are retained to identify older security settings and are not advice to disable Enhanced Protection. If installation is blocked, verify app provenance, signature, and permissions first. When no official installation path works, choose a tool supported by the current system rather than bypassing device security policy.
