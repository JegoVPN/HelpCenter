...
When **Current location** shows Auto Select (see below), Jego picks the route for you:
...
If only one site fails, run [Rules Check](/en/guide/network-diagnostics#route-check) and see whether the domain is proxied. An available node does not mean every site automatically uses it.
