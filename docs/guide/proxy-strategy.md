...
3. 在**代理线路**中选择一种连接方式：
   ...
   - 选择**直连（不使用代理）**，匹配的地址就会使用本地网络。
4. 在**域名/主机列表**中填写网站域名，例如 `bbc.com`。多项内容可以一行一个；有明确需求时，也可以填写 IPv4 地址或使用 `1.1.1.*` 这样的 IPv4 网段（网段写法用于直连规则）。
...
- **按 IP 分流**：单个 IPv4 地址可以搭配节点或直连；`1.1.1.*` 这样的网段写法仅在代理线路选择**直连（不使用代理）**时生效，暂不支持让整段网段走指定节点。
...
- **哔哩哔哩使用本地网络**：代理线路选择**直连（不使用代理）**，域名/主机列表填写 `bilibili.com`。
...
详细的检查方法见[查网址走向](/guide/network-diagnostics#route-check)。

## AI 产品

这些服务通常先使用无忧行现有的默认策略，选择正确的线路即可，详细使用方法见[如何使用无忧行访问 AI 产品](/guide/chatgpt-access)。
