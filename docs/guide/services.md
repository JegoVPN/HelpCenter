---
translationKey: guide-services
contentType: reference
product: general
productArea: network-reference
uiSurface: null
locale: zh-Hans
status: current
owner: docs
reviewStatus: needs-review
lastVerified: null
platforms: [chrome, edge]
tools: []
appliesTo: []
sources: []
title: 无忧行的三个版本 - 使用指南
description: 轻松了解无忧行免费版、限时体验版和付费版的主要区别。
---

# 无忧行的三个版本

打开无忧行插件弹窗，右上角可以看到当前版本。每个版本在网站范围、节点、代理策略和设备支持上都有区别，按当前版本查看对应说明即可。

## 免费版

免费版可以直接在 Chrome 和 Edge 浏览器中使用，但可访问的网站有限，目前主要支持 Google、Wikipedia、NewBing、ChatGPT、Grok、Claude 等指定网站。

免费版使用境外边缘网络，节点和代理策略由无忧行预先设置；电脑和手机订阅服务在会员版中提供。

<img src="/images/jego-v1.5.9/popup-free-on-zh.png" alt="免费版本界面" width="280">

## 体验版

注册并通过欢迎邮件激活后，可以获得三天体验。体验版的网站范围不受免费版指定名单限制，可以从几个节点中选择并自定义代理策略，还可以使用境外大陆优化线路；电脑和手机订阅服务仅会员版提供。

<img src="/images/jego-v1.5.9/popup-trial-rules-auto-zh.png" alt="限时体验界面" width="280">

## 会员版

会员版的网站范围同样不受免费版指定名单限制，可以选择全球几十个高速节点、自定义代理策略，并使用境外边缘网络、境外大陆优化线路和内网专线。

会员还包含电脑和手机订阅服务。完整权益见[会员体系](/membership/benefits)，付款步骤见[如何支付](/membership/payment)。

<img src="/images/jego-v1.5.9/popup-paid-rules-auto-zh.png" alt="付费服务界面" width="280">
