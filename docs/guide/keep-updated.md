...
sources: []
title: 防止失联 - 使用指南
description: 查看无忧行当前版本，并按浏览器或手机客户端完成更新。
---

# 防止失联

无忧行会持续更新插件版本和订阅内容。保持更新后，浏览器和手机客户端可以及时使用新的节点与连接设置。商店安装的插件通常会自动更新；需要立即确认时，按本页对应的浏览器步骤操作即可。
...
2. 在地址栏输入 `chrome://extensions/`，打开扩展管理页，点击左上角的**更新**。
...
2. 在地址栏输入 `edge://extensions/`，打开扩展管理页，点击右上角的**更新**。
...
手机上的订阅客户端建议每 24 小时更新一次，以便及时获取无忧行更新的节点和端口。续费或账户里的订阅内容发生变化后，也可以立即更新一次。

不同客户端的更新按钮会有所不同，可以从[按设备安装](/subscription/#按设备安装)进入正在使用的客户端教程。客户端显示新的更新时间和状态后，更新就完成了。

## 更新完成后

浏览器插件更新完成后，完全退出浏览器再重新打开。需要核对版本或重新确认模式和节点时，继续查看[插件更新与恢复](/guide/plugin-maintenance)。

## 联系支持

需要人工协助时，可以直接[联系支持](/guide/support)，并说明正在使用的浏览器或客户端名称及当前版本。
