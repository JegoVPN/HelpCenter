...
- **活动中心**：查看当前账号可以参加的活动。
- **联系客服**：提交问题并查看客服回复。
- **购买会员 / 会员续费**（随会员状态显示其一）：查看当前可购买或续费的会员方案。
- **支付记录**：查看已经创建的订单和付款状态。
- 页面底部的**使用指南**会打开面板内的指南页，从中可以进入帮助中心；**联系客服**可以直接进入支持入口。
- 点击左栏底部的账号邮箱可以展开账号菜单：查看会员信息、切换外观、进入**账户管理**或**退出登录**。

## 常见操作从这里开始

- 查看一个网址会走代理还是直连：[查网址走向](/guide/network-diagnostics#route-check)
- 比较当前账号里的节点：[节点测速](/guide/network-diagnostics-node-speed)
- 让指定网站按自己的规则连接：[自定义代理策略](/guide/proxy-strategy)
- 在电脑或手机的其他应用中使用订阅：[订阅服务](/subscription/)（打开后按设备选择对应教程）
